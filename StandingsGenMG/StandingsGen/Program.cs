...
namespace StandingsGen
{
    class Program
    {
        ...
        {
            var file = "tournaments//МСХЛА2018";
            Presets(file);
            DrawingTable();
        ...
        private static void DrawingTable()
        {
            ...
            Image imgTableLayer  = Image.FromFile("images//tablelayer.png");
            Image imgRect  = Image.FromFile("images//rect.png");
            ...
            using (Graphics g = Graphics.FromImage(bitmap))
            {
                ...
                g.DrawImage(imgTableLayer, rectTableBckgr);
                ...
                var imgAvi = Image.FromFile("teams//мгюа.png");
                ...
                var heightTableName = sizeLigaLogo - marginUpDownLigaLogo + 100;

                xTableName += widthTableName / 6;
                yTableName -= 25;

                var rectTableName = new Rectangle(xTableName, yTableName, widthTableName, heightTableName);

                var brushWhite = Brushes.White;

                var fileFontTableName = "fonts//12261.ttf";
                var sizeFontTableName = 120;
                var fontTableName = CreateFont(fileFontTableName, sizeFontTableName);

                var fmt = new Format();
                var centerFormat = fmt.centerFormat;

                DrawOutlineText(g, sett.NameTable, fontTableName, rectTableName, brushWhite, Color.Black);
                ...
                var rectInscSocial = new Rectangle(widthBitmap / 2 - imgBorn.Width / 2,
                    ...
                    bitmap.Width * 2 / 3, 150);
                //g.DrawImage(imgBorn, GetInscribed(rectInscSocial, imgBorn.Size));
                ...
                var fileFontHat = "fonts//12228.ttf";
                var sizeFontHat = 33;
                ...
                if (sett.PartScores)
                {
                    ...
                    for (int i = 0; i < teamsCnt; ++i)
                    {
                        var rectHatTableScores = new Rectangle(xHatTableScores + i * widthScore, yHatTable, widthScore,
                            heightHat);
                        g.DrawString((i + 1).ToString(), fontHat, brushWhite, rectHatTableScores, centerFormat);
                ...
                for (int i = 0; i < sett.ListIndicators.Count; i++)
                {
                    ...
                    g.DrawString(nameInd, fontHat, brushWhite, rectHatTableIndic, centerFormat);
                ...
                if (sett.PartNames)
                {
                    var fileFontNames = "fonts//12228.ttf";
                    var fileFontNamesForAvi = "fonts//12228.ttf";
                    ...
                    for (int i = 0; i < teamsCnt; i++)
                    {
                        ...
                        var brushName1 = brushWhite;

                        var brushName2 = brushWhite;

                        Font fontTeamName = null;
                        Font fontTeamInst = null;

                        if (full.nameTeam.Contains("легион"))
                ...
                if (sett.PartScores)
                {
                    for (int i = 0; i < teamsCnt; ++i)
                    {
                        ...
                        var rectRect = new Rectangle(xLogo + i * widthScore - 7, yLogo + i * heightTableRow - 5, 100,
                            100);

                        var inscLogo = GetInscribed(rectLogo, imgLogo.Size);
                        var inscRect = GetInscribed(rectRect, imgRect.Size);


                        g.DrawImage(imgRect, inscRect);
                        g.DrawImage(imgLogo, inscLogo);
                ...
                if (sett.PartScores)
                {
                    var fontWin = "fonts//12228.ttf";
                    var fontOt = "fonts//12228.ttf";
                    var fontLose = "fonts//12228.ttf";
                    ...
                    for (int i = 0; i < teamsCnt; ++i)
                    {
                        for (int j = 0; j < teamsCnt; ++j)
                        {
                            ...
                            if (sett.GamesBetween == 1)
                            {
                                ...
                                g.DrawString(strScore, fontScore, brushWhite, rectScore, centerFormat);
                                ...
                                if (workGame.IsOt > 0)
                                {
                                    ...
                                    g.DrawString(strPorOt, fontOtPen, brushWhite, rectOt, centerFormat);
                                }
                            }
                ...
                for (int i = 0; i < teamsCnt; ++i)
                {
                    ...
                    for (int k = 0; k < sett.ListIndicators.Count; ++k)
                    {
                        ...
                        g.DrawString(strIndic, fontInd, brushWhite, rectIndic, centerFormat);
                    }
                }
        ...
        static void DrawOutlineText(Graphics g, String text, Font font, Rectangle r, Brush b, Color line)
        {
            // set atialiasing
            g.SmoothingMode = SmoothingMode.HighQuality;

            // make thick pen for outlining
            Pen pen = new Pen(line, 5);
